...
def _get_profile(config):
    profiles = [
        teleop_lib.input_profile.build_profile(cfg) for cfg in config.get("modes", [])
    ]
    ...
    if len(profiles) == 0:
        ...
        # TODO: warn if no mode switch assigned but using modal control
        return teleop_lib.input_profile.ModalControlMap(profiles)

def _get_plugin(config):
